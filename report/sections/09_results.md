...
| 严格样本外（阈值 1.00） | 测试段 2 笔，PnL +2655.53，年化 Sharpe 1.42，Bonferroni α=0.01（样本不足，如实标记） | `evaluation/test_evaluation_log.json`（`scripts/run_strict_evaluation.py` 锁定运行） |
...
训练/验证段选择、测试段锁定，跨边界交易从选择中排除）最新锁定运行选出
阈值 1.00，测试段 2 笔取得 +2655.53（年化 Sharpe 1.42，样本不足如实标记）。

## 9.3 不确定性度量与诚实声明

- 样本很小：滚动交易 6–33 笔、测试段 2 笔，统计功效低，置信区间宽
